...
local customConfig = {
	...
	termguicolors = true,
	-- tabs, indenting
	tabstop = 4, -- tab space value when reading a file
	...
	sidescrolloff = 10, -- same for horizontal scrolling
...
-- custom helpers for my crappy laptop:

--[[ local matebookBrokenKeyboardKeymaps = {
	slash = { modes = { "n", "i", "v" }, command = "<M-.>", result = "/", desc = "Alt+. for Forward Slash" },
	question = {
		modes = { "n", "i", "v" },
		command = "<M->>",
		result = "?",
		desc = "Alt+> for Question Mark",
	},
	z = { modes = { "n", "i", "v" }, command = "<M-x>", result = "z", desc = "Alt+x for z" },
	Z = { modes = { "n", "i", "v" }, command = "<M-X>", result = "Z", desc = "Alt+X for Z" },
	four = { modes = { "n", "i", "v" }, command = "<M-5>", result = "4", desc = "Alt+5 for 4" },
	Four = { modes = { "n", "i", "v" }, command = "<M-%>", result = "$", desc = "Alt+% for $" },
	del = {
		modes = { "n", "i", "v" },
		command = "<M-BACKSPACE>",
		result = "<DEL>",
		desc = "Alt+Backspace for DELETE",
	},
}

for _, v in pairs(matebookBrokenKeyboardKeymaps) do
	for _, mode in pairs(v.modes) do
		vim.keymap.set(mode, v.command, v.result, { desc = v.desc, noremap = opts.noremap, silent = opts.silent })
	end
end ]]

-- vim.keymap.set("n", "<leader>.", "/", opts)
-- vim.keymap.set("n", "<leader>..", "?", opts)
-- vim.keymap.set("i", "<leader>.", "/", opts)
-- vim.keymap.set("i", "<leader>..", "?", opts)
-- vim.keymap.set("v", "<leader>.", "/", opts)
-- vim.keymap.set("v", "<leader>..", "?", opts)
--
-- vim.keymap.set("n", "<leader>x", "z", opts)
-- vim.keymap.set("n", "<leader>xx", "Z", opts)
-- vim.keymap.set("i", "<leader>x", "z", opts)
